...
describe('PersistStorage', () => {
	it('PersistStorage.set', () => {
		...
		return PersistStorage.set({ key1: 'string', key2: 123, key3: true, key4: { foo: 'bar' }, key5: [1, 'a'] }).then(
			result => {
				expect(result).toEqual({ key1: 'string', key2: 123, key3: true, key4: { foo: 'bar' }, key5: [1, 'a'] });
			},
		);
	});
