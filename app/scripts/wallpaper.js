...
(function() {
	...
	nau.wallpaper = {
		init(selector) {
			...
			let imgUrl = this.imgUrl = localStorage.getItem('imgUrl');
			const DEBUG = false;
			...
			if (imgUrl) {
				...
				this.imgUrl = 'images/bg-default.jpg';
				this.render();
			}
		},
		...
		render() {
			...
			if (localStorage.getItem('authorUsername')) {
				$('#photo-credit').innerHTML = `
					Photo by
					<a href="https://unsplash.com/@${localStorage.getItem('authorUsername')}"
						class="photo-credit__author">${localStorage.getItem('authorName')}</a>
					/
					<a href="https://unsplash.com/photos/${localStorage.getItem('imgId')}"
